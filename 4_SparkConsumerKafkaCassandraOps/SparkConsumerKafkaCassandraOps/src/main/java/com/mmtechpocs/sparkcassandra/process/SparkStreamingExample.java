...
import com.mmtechpocs.sparkcassandra.model.Vote;
...
//			    );
		...
		voteCount.foreachRDD((v1, v2) -> {
			...
			v1.foreach((x) -> {
				...
				CassandraTableScanJavaRDD<CassandraRow> previousVotes = javaFunctions(sc).cassandraTable("voting", "votes").where("name = '" + x._1() + "'");
